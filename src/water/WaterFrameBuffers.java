...
public class WaterFrameBuffers {
	...
	// Water effects will be carried on using frame buffer objects. Frame buffer
	// objects allow rendering inside them instead of rendering into the default
	// frame buffer (and into the display once the update display method is called).
	// Once there are some graphics being rendered into a frame buffer object, their
	// contents can be used as a texture and this means that we can decorate some
	// entity with this texture.
	// As we want to simulate water, we need to simulate refraction and reflection.
	// We will have one frame buffer object for each of the effects to simulate and
	// each of the frame buffer objects will be used to generate textures.
	// Frame buffer objects contains different buffers: color buffer (stating the 
	// color of each pixel), depth buffer (stating the depth in space for each pixel),
	// stencil buffer, etc. For water simulation we will be needing the color buffer
