...
public class ShadowBox {
	...
	protected void update() {
		...
		toNear.scale(MasterRenderer.getNearPlane());
	...
	private void calculateWidthsAndHeights() {
		farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(MasterRenderer.getFieldOfView())));
		nearWidth = (float) (MasterRenderer.getNearPlane()
				* Math.tan(Math.toRadians(MasterRenderer.getFieldOfView())));
		farHeight = farWidth / (float)DisplayManager.getAspectRatio();
		nearHeight = nearWidth / (float)DisplayManager.getAspectRatio();
	}
